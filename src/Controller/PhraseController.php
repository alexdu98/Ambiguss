...
use App\Form\Phrase\PhraseAddType;
...
class PhraseController extends AbstractController
{
	public function new(Request $request, LoggerInterface $logger)
	{
        ...
        if($form->isSubmitted() && $form->isValid()) {
            ...
            if($succes) {
                ...
                $event = new GenericEvent(GameEvents::PHRASE_CREEE, array(
                    'membre' => $this->getUser(),
                    'phrase' => $newPhrase
                ));
                $ed->dispatch(GameEvents::PHRASE_CREEE, $event);
            }
            else {
        ...
        return $this->render('App:Phrase:add.html.twig', array(
	...
	public function edit(Request $request, LoggerInterface $logger, Phrase $phrase)
	{
        $secu = $this->get('security.authorization_checker');
        $bag = $this->get('session')->getFlashBag();
        ...
        $dureeAvantJouabiliteS = $this->getParameter('app.dureeAvantJouabiliteSecondes');
        ...
        if($form->isSubmitted() && $form->isValid() && !$timeExceeded) {
            ...
            if($succes) {
                ...
                $ed = $this->get('event_dispatcher');
                $event = new GenericEvent(GameEvents::PHRASE_MODIFIEE, array(
                    'membre' => $this->getUser(),
                    'phrase' => $newPhrase
                ));
                $ed->dispatch(GameEvents::PHRASE_MODIFIEE, $event);
            }
            else {
        ...
        return $this->render('App:Phrase:edit.html.twig', array(
            'form' => $form->createView(),
            'phrase' => $phrase,
            'phraseOri' => $phraseOri,
            'reponsesOri' => $repOri,
