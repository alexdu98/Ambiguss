...
class BadgeService
{
    ...
    public function check(Membre $membre, $type)
    {
        ...
        switch ($type) {
            ...
            case 'SIGNALEMENT_VALIDE_TOTAL':
                $signalementRepo = $this->em->getRepository('AppBundle:Signalement');
                $repoTV = $this->em->getRepository('AppBundle:TypeVote');
    ...
    {
        $notifier = $this->container->get('AppBundle\Service\NotifyService');
        $historizer = $this->container->get('AppBundle\Service\HistoriqueService');
