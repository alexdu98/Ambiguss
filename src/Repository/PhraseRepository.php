...
class PhraseRepository extends ServiceEntityRepository
{
    ...
    public function getClassementPhrases(int $limit){
	    return $this->createQueryBuilder('p')->select("p.id, p.contenu, p.dateCreation, p.gainCreateur")->distinct()
		    ->addSelect('(SELECT COUNT(lp2.id) FROM App\Entity\JAime lp2 WHERE lp2.phrase = p.id AND lp2.active = 1) as nbJAime')
	...
	public function getClassementPhrasesUser(Membre $user)
	{
		return $this->createQueryBuilder('p')
            ...
            ->distinct()
			->addSelect('(SELECT COUNT(lp2.id) FROM App\Entity\JAime lp2 WHERE lp2.phrase = p.id AND lp2.membre != :user AND lp2.active = 1) as nbJAime')
			->leftJoin("p.jAime", "lp", 'WITH', 'lp.id = p.id')
	...
	public function findIdPhrasesNotPlayedByMembre(Membre $membre, int $dureeAvantJouabiliteSecondes)
	{
		...
		$sub = $this->_em->getRepository('App:Partie')->createQueryBuilder('pa')
